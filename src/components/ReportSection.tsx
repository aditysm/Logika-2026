...
export function ReportSection({
  ...
  // Delete / Reset Report Modal state
  const [isDeleteReportModalOpen, setIsDeleteReportModalOpen] = useState<boolean>(false);
  const [deleteReportReason, setDeleteReportReason] = useState<string>('Ada file foto yang salah di dalam laporan');
  const [deleteReportCustomReason, setDeleteReportCustomReason] = useState<string>('');

  const reportReasonPresets = [
    'Ada file foto yang salah di dalam laporan',
    'Ingin membuat ulang laporan baru',
    'File rusak / tidak bisa dibuka / tidak bisa diakses',
    'Biodata / urutan kelompok keliru',
    'Lainnya (isi manual)',
  ];

  const handleSendDeleteReportToAdmin = () => {
    if (!currentUser.nim) return;

    const reasonText =
      deleteReportReason === 'Lainnya (isi manual)'
        ? (deleteReportCustomReason.trim() || 'Lainnya')
        : (deleteReportCustomReason.trim() ? `${deleteReportReason} (${deleteReportCustomReason.trim()})` : deleteReportReason);

    const docUrl = reportData?.pdf_url || '-';

    const message = `Halo Admin, saya ingin mengajukan permohonan penghapusan / reset dokumen laporan Word di Logika 2026.

Rincian Dokumen Laporan:
- NIM: ${currentUser.nim}
- Nama Lengkap: ${currentUser.namaLengkap}
- Kelompok: ${currentUser.kelompok}
- Tautan Dokumen: ${docUrl}

Alasan saya ingin menghapus/reset laporan ini karena: ${reasonText}`;
    ...
    <div className="space-y-4">
      ...
      >
        ...
        {status === 'failed' && reportData?.error_message && (
          <div className="relative z-10 text-xs text-rose-800 bg-rose-50 border border-rose-200 rounded-xl p-3 flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-rose-600 shrink-0 mt-0.5" />
            <span>Keterangan: {getIntuitiveReportErrorMessage(reportData.error_message)}</span>
          </div>
        )}
        ...
        <div className="relative z-10 pt-2 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          {isLoadingStatus ? (
            ...
            <div className="flex-1 flex flex-col items-start gap-3">
              ...
              {/* Reset / Hapus Laporan link option for Completed Reports */}
              {isCompleted && (
                <p className="text-[11px] text-slate-500 pt-0.5">
                  Dokumen salah?{' '}
                  <button
                    id="btn-open-delete-report-modal"
                    type="button"
                    onClick={() => setIsDeleteReportModalOpen(true)}
                    className="text-rose-600 hover:text-rose-700 font-bold underline underline-offset-2 hover:underline-offset-4 transition-all cursor-pointer"
                  >
                    Ajukan Penghapusan / Reset Laporan
                  </button>
                </p>
              )}
            </div>
      ...
      {/* Modal Permohonan Penghapusan / Reset Dokumen Laporan */}
      {isDeleteReportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="relative w-full max-w-lg bg-white border border-slate-200 rounded-3xl p-6 sm:p-7 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base sm:text-lg font-bold text-slate-900">
                    Ajukan Penghapusan / Reset Dokumen
                  </h3>
                  <p className="text-xs text-slate-500">
                    Permohonan Hapus / Buat Ulang Laporan ke WA Admin
                  </p>
                </div>
              </div>
              <button
            ...
            {/* Modal Body: Info Mahasiswa & Dokumen */}
            <div className="space-y-3">
              <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-2 text-xs">
                ...
                {reportData?.pdf_url && (
                  <div className="flex items-center justify-between border-t border-slate-200/60 pt-2">
                    <span className="text-slate-500 font-medium">Berkas Dokumen:</span>
                    <a
                      href={reportData.pdf_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-600 hover:underline inline-flex items-center gap-1 font-semibold truncate max-w-[200px]"
                    >
                      <span>Buka File Word / Drive</span>
              ...
              {/* Alasan Dropdown & Manual Input */}
              <div className="space-y-1.5 pt-1">
                <label htmlFor="select-delete-report-reason" className="block text-xs font-bold text-slate-800">
                  Alasan saya ingin menghapus / mereset laporan ini karena:
                </label>
                <select
                  id="select-delete-report-reason"
                ...
                <textarea
                  rows={2}
                  value={deleteReportCustomReason}
                  onChange={(e) => setDeleteReportCustomReason(e.target.value)}
                  placeholder={
                    deleteReportReason === 'Lainnya (isi manual)'
                      ? 'Silahkan ketik alasan detail mengapa dokumen ingin dihapus / direset...'
                      : 'Tambahan rincian alasan (opsional)...'
                  }
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
              ...
              <button
                type="button"
                onClick={handleSendDeleteReportToAdmin}
